...
from cl_monitoring.domain.models import RunSummary, RunResult, Confidence, ErrorFamily

def parse_crawllib_logs(task_id: str, execution_key: str, log_lines: Iterable[str]) -> RunSummary:
    counters = {
        "item_events": 0,
        "put_to_parser": 0,
        "is_success_true": 0,
    ...
    for line in log_lines:
        ...
        # put_to_parser
        if "put_to_parser" in line_clean and ("INFO:crawllib.managers:" in line_clean or "INFO:HOST_" in line_clean):
        ...
        # summary stats
        if "Статистика" in line_clean:
            has_summary_stats = True
            
        # cancel markers
        if "CancelledError:" in line_clean or "{'status': 'ok', 'message': 'success', 'total': 0, 'data': None, 'error': ''}" in line_clean:
            has_cancel = True
            evidence.append(line_clean)
        ...
        # 429 ban
        if "ban status code 429" in line_clean.lower() or "429" in line_clean and "ban" in line_clean.lower():
            has_429_ban = True
            evidence.append(line_clean)
